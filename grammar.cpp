...
Grammar::Grammar(char start,
                 const vector<string> &rules) : _start(start)
{
    if (isTerm(start)) {
...
void Grammar::parseRules(vector<string> rules)
{
    for (int i = 0; i < rules.size(); ++i)
    {
        ...
        // TODO
        // не проверяю, есть ли правила для всех нетерминалов
        // TODO построить массивы всех терминалов и нетерминалов, чтобы потом заполнять пустыми множествами все нетерминалы
...
bool Grammar::checkWord(string word, string& message) {
    ...
    beg->child = new Node(end);
    nodes.push_back(beg);
    nodes.push_back(beg->child);
        ...
        for (auto p_cur_node : cur_nodes) {
            ...
            if (isTerm(cur_node->s)) {
                ...
                if (lenta_elem == cur_node->s) {
                    ...
                    new_cur_nodes.insert({cur_node->child, new_lenta_ind});
                    error_ind = max(error_ind, new_lenta_ind); // маскимальный достижимый индекс
                } else {
                    // is_final = true;
                    // message = "wrong symbol in position " + p_cur_node.second;
                    // break;
                }
            } else {
